...
function printFormattedDocument(
  filename: string,
  ...
  switch (options.writeMode) {
    ...
    case "replace":
      ...
      return writeFileAsync(filename, formattedDocument).then(() => {
...
if (program.stdin) {
  getStdin()
    .then((input: string) => {
      return printFormattedDocument(
        "<stdin>",
        input,
        formatText(input, customOptions),
        customOptions
      );
    })
    .then(() => {
