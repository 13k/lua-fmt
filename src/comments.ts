  ...
  function addChild(n: luaparse.Node) {
    if (n && typeof n.type === "string" && n.type !== "Comment") {
      ...
      for (idx = children.length - 1; idx >= 0; --idx) {
        if (locStart(children[idx]) <= locStart(n) && locEnd(children[idx]) <= locEnd(node)) {
          break;
        }
      }
...
export function attachComments(ast: luaparse.Chunk, options: Options): void {
...
export function injectShebang(ast: luaparse.Chunk, options: Options): void {
...
export function printDanglingComments(path: FastPath, sameIndent = false): Doc {
...
function printLeadingComment(path: FastPath, options: Options) {
  ...
  if (isBlockComment) {
    return concat([comment.raw, hasNewLine(options.sourceText, locEnd(comment)) ? hardline : " "]);
  }
...
export function printComments(path: FastPath, options: Options, print: PrintFn): Doc {
  const node = path.getValue();
...
function decorateComment(node: luaparse.Node, comment: luaparse.Comment) {
  ...
  while (left < right) {
    ...
    if (locStart(childNode) - locStart(comment) <= 0 && locEnd(comment) - locEnd(childNode) <= 0) {
...
function handleIfStatementsWithNoBodyComments(
  ...
  if (
    followingNode &&
    (followingNode.type === "ElseifClause" || followingNode.type === "ElseClause")
  ) {
