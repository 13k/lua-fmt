...
function any(insn: Doc, callback: Callback): boolean {
  let result = false;
...
export function willBreak(insn: Doc): boolean {
...
function breakParentGroup(stack: Group[]): void {
  if (stack.length > 0) {
    stack[stack.length - 1].willBreak = true;
  }
}

export function propagateBreaks(insn: Doc): void {
